 ...
 * Used to verify CRC
 */
//sunitec提供的dfu crc计算工具类
public class DfuCRC {
	private static final long[] revCrcTable = { 0x00000000, 0xDB710641,
    ...
    /**
     * Function to compute crc from byte[]
     * @param data byte[]
    ...
    /**
     * Function to return CRC value of file from byte[]
     * @param data byte[]
     * @return long file crc
     */
	public static long fileCrc(byte[]data) {
		if(data==null) return -1;
		try {
			long fileLen=data.length;
			byte[] buffer = new byte[(int) fileLen + 4];
			
			buffer[0] = buffer[1] = buffer[2] = buffer[3] = (byte) 0xff;
			System.arraycopy(data, 0, buffer, 4, buffer.length - 4);
	...
	/**
	 * Function to return CRC value of file from InputStream
	 * @param stream {@link InputStream}
	 * @return long file crc byte
	 */
	public static long fileCrc(InputStream stream) {
